...
/// struct for typed errors of method [`server_impact_by_config_v1_server_bottom_up_post`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ServerImpactByConfigV1ServerBottomUpPostError {
...
pub async fn server_impact_by_config_v1_server_bottom_up_post(configuration: &configuration::Configuration, verbose: Option<bool>, server_dto: Option<crate::models::ServerDto>) -> Result<serde_json::Value, Error<ServerImpactByConfigV1ServerBottomUpPostError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/v1/server/bottom-up", local_var_configuration.base_path);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<ServerImpactByConfigV1ServerBottomUpPostError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
